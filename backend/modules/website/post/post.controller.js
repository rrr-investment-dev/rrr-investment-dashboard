...
import { createPost as createPostService, getAllPosts as getAllPostsService, getPostById as getPostByIdService, updatePost as updatePostService, togglePostStatus as togglePostStatusService, deletePost as deletePostService, getPublishedPosts as getPublishedPostsService, getPublishedPostById as getPublishedPostByIdService } from "./post.service.js";
...
export const createPost = catchAsync(async (req, res, next) => {
    ...
    if (!title || !link || !creatorId) {
        return next(new AppErrorClass("Title, link, and creator ID are required", 400));
    }
...
export const togglePostStatus = catchAsync(async (req, res, next) => {
    ...
    res.status(200).json({ status: "success", message: `${post.title} ${post.status.toUpperCase()} successfully`, data: { post } });
});
